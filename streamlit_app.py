...
if menu == "🏠 Nuovo Intervento":
    ...
    with st.container():
        ...
        km_att = st.number_input("Chilometri rilevati sul mezzo", value=safe_int(df.at[idx, 'KM_Attuali']), step=1)
        ...
        with col1:
            st.markdown(f"<div class='scadenza-box tagliando'><small>TAGLIANDO A</small><br><b style='font-size:24px;'>{km_prossimo_t} km</b></div>", unsafe_allow_html=True)
        with col2:
            st.markdown(f"<div class='scadenza-box gomme'><small>GOMME A</small><br><b style='font-size:24px;'>{km_prossimo_g} km</b></div>", unsafe_allow_html=True)

    # Checklist
    with st.container():
        st.markdown("### Lavori Eseguiti")
        c1, c2 = st.columns(2)
        check_t = c1.checkbox("⚙️ Tagliando completato")
        check_g = c2.checkbox("🛞 Cambio gomme completato")
        
        note_val = df.at[idx, 'Altro']
        if note_val.lower() == "nan" or note_val == "": note_val = ""
        
        altro = st.text_area("Note aggiuntive", value=note_val)

    # Salva
    if st.button("💾 SALVA E GENERA REPORT PDF", use_container_width=True, type="primary"):
...
# --- ADMIN ---
    ...
    st.markdown("<h1>📋 Stato Flotta</h1>", unsafe_allow_html=True)
