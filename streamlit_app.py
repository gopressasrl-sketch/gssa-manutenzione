...
def process_image(uploaded_file):
    """COMPRESSIONE MASSIMA PER EVITARE ERRORI API"""
...
lista_mezzi = sorted(df_man['Targa'].unique().tolist()) if not df_man.empty else ["GG730AV"]
...
# --- 7. PAGINE INTERNE ---
    ...
    for targa in df_seg[df_seg['Stato'] == 'APERTO']['Targa'].unique():
        with st.expander(f"🚛 PANNE: {targa}", expanded=True):
            ...
            c = st.columns(6); fl = ["Gom", "Spi", "KM", "Tar", "Lib"]; fc = ["Foto_Gomme", "Foto_Cruscotto", "Foto_KM", "Foto_Targa", "Foto_Libretto"]
            ...
            if st.button(f"📧 INVIA MAIL {targa}"):
                ...
                if invia_email_ufficiale(rub_dict.get(sel_m,""), targa, dg['KM_Segnalazione'], dg['Descrizione'], fa):
                    st.success("MAIL INVIATA!")
                    # AUTO-CLEAN: Cancella le foto dal database dopo l'invio per liberare spazio
                    df_seg.loc[(df_seg['Targa'] == targa) & (df_seg['Stato'] == 'APERTO'), fc] = "FOTO INVIATA"
