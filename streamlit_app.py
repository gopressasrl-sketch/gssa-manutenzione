...
# Inizializzazione sessione (TUTTE LE FUNZIONI ATTIVE)
...
# --- 2. SUPER CSS IPHONE 17 PRO MAX PLATINUM ---
st.markdown(f"""
    <style>
    ...
    .dynamic-island {{
        background: rgba(28, 28, 30, 0.8); border-radius: 50px;
    ...
    .island-status {{ color: #8e8e93; font-size: 0.9em; margin-top: 5px; text-transform: uppercase; letter-spacing: 1px; }}

    /* GRIGLIA APP iPHONE (4 COLONNE) */
    [data-testid="stHorizontalBlock"] {{ max-width: 420px !important; margin: 0 auto !important; }}
    [data-testid="column"] {{ display: flex !important; flex-direction: column !important; align-items: center !important; justify-content: center !important; padding: 0 !important; }}
    ...
    .stButton>button {{
        ...
        margin: 0 auto !important; box-shadow: 0 10px 20px rgba(0,0,0,0.5) !important;
    ...
    div[data-testid="column"]:nth-child(3) .stButton>button {{ background: #2c2c2e !important; border: 0.5px solid #d4af37 !important; }}
    ...
    .app-label {{ color: white; font-size: 11px; margin-top: 6px; text-align: center; font-weight: 400; width: 80px; white-space: nowrap; margin-bottom: 25px; }}
    ...
    .status-card-inner {{ text-align: center; background: #000; border-radius: 20px; padding: 15px; margin-bottom: 15px; border: 1px solid #d4af37; }}
    .status-val {{ font-size: 26px; font-weight: 700; color: #d4af37; }}
    </style>
...
def process_image(uploaded_file):
    """SALVATAGGIO STABILE: Compressione a 400px per evitare crash Google"""
...
if not st.session_state.user:
    st.markdown(f'<div class="dynamic-island"><h1 class="island-title">GOPRESSA</h1><p class="island-status">Identification Required</p></div>', unsafe_allow_html=True)
...
# (Pagine admin, manutenzione, danno e status caricate come prima ma stabili)
    ...
    st.divider(); ts = st.selectbox("STORICO PDF", lista_mezzi); df_sto_v = carica_dati("Storico")
