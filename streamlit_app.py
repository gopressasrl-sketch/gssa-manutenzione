...
# --- 2. SUPER CSS: DESIGN ORIZZONTALE & PULIZIA ---
st.markdown(f"""
    <style>
    ...
    /* DESIGN ORIZZONTALE (ICONA + BOTTONE) */
    ...
    .icon-box {{
        width: 60px; height: 60px; background: #1c1c1e; border-radius: 18px;
        display: flex; align-items: center; justify-content: center;
        font-size: 1.8em; box-shadow: 0 4px 10px rgba(0,0,0,0.5);
    }}
...
if not st.session_state.user:
    st.markdown(f'<div class="ios-pill-container"><h1 class="main-title">GOPRESSA</h1><p style="color:gray;">{ora_it}</p></div>', unsafe_allow_html=True)
...
# --- 5. HEADER ---
st.markdown(f'<div class="ios-pill-container"><h1 class="main-title">GOPRESSA</h1><p style="color:gray;">{st.session_state.user} | {ora_it}</p></div>', unsafe_allow_html=True)
...
# --- 7. PAGINE ---
    ...
    st.divider(); df_sto_v = df_sto.copy(); ts = st.selectbox("ARCHIVIO PDF", lista_mezzi)
